...
public class InductiveCutMiner extends AbstractPetrinetModel implements ICutsProcessTreeMiner {
    ...
    public InductiveCutMiner(String filename) throws Exception {
        super(filename);
    }
    ...
    public ProcessTreeCuts Mine(XLog xLog) {
        ...
        //create process tree
        ProcessTreeCuts res = new ProcessTreeCuts();
        ProcessTree tree = res.processTree;
    ...
    public static Node mineNode(IMLog log, ProcessTreeCuts ptCuts, MinerState minerState) {
        ProcessTree tree = ptCuts.processTree;
        ...
        if (baseCase != null) {
            ...
            baseCase = postProcess(ptCuts, baseCase, log, logInfo, minerState);
        ...
        if (cut != null && cut.isValid()) {
            ...
            if (cut.getOperator() != Cut.Operator.loop) {
                for (IMLog sublog : splitResult.sublogs) {
                    Node child = mineNode(sublog, ptCuts, minerState);
                ...
                {
                    Node firstChild = mineNode(firstSublog, ptCuts, minerState);
            ...
            Node result = postProcess(ptCuts, newNode, log, logInfo, minerState);
            ...
            result = postProcess(ptCuts, result, log, logInfo, minerState);
    ...
    private static Node postProcess(ProcessTreeCuts ptCuts, Node newNode, IMLog log, IMLogInfo logInfo, MinerState minerState) {
        ...
        ptCuts.logs.put(newNode.getID(), log.toXLog());
