...
public class AdaptiveNoiseExhaustive extends AbstractPetrinetMiner {
    ...
    private Map<Float, MiningParametersIMf> parametersIMfMap;
    ...
    public AdaptiveNoiseExhaustive(String filename, NoiseThreshold noiseThreshold) throws LogFileNotFoundException {
        ...
        for (float threshold: thresholds) {
            this.parametersIMfMap.put(threshold, new MiningParametersIMf() {{ setNoiseThreshold(threshold);}});
        }
    }
    ...
    public List<Node> mineNode(IMLog log, ProcessTree tree, MinerState minerState) {
        ...
        for(Map.Entry<Float, MiningParametersIMf> mfEntry: parametersIMfMap.entrySet()) {
            Cut cut = findCut(log, logInfo, minerState, mfEntry.getValue());
    ...
    private List<Node> handleCut(IMLog log, IMLogInfo logInfo, Cut cut, MiningParameters miningParameters, MinerState minerState, ProcessTree originalTree) {
        ...
        if (cut != null && cut.isValid()) {
            ...
            //split logs
            LogSplitter.LogSplitResult splitResult = splitLog(log, logInfo, cut, miningParameters, minerState);
            ...
            ProcessTree tree = originalTree.getRoot() != null ? originalTree.toTree() : new ProcessTreeImpl();
            Node result = findFallThrough(log, logInfo, tree, miningParameters, minerState);
    ...
    public static Cut findCut(IMLog log, IMLogInfo logInfo, MinerState minerState, MiningParameters miningParameters) {
        Cut c = null;
    ...
    public static Node findFallThrough(IMLog log, IMLogInfo logInfo, ProcessTree tree, MiningParameters miningParameters, MinerState minerState) {
        Node n = null;
        Iterator<FallThrough> it = miningParameters.getFallThroughs().iterator();
        while (n == null && it.hasNext()) {
    ...
    public static LogSplitter.LogSplitResult splitLog(IMLog log, IMLogInfo logInfo, Cut cut, MiningParameters miningParameters, MinerState minerState) {
        LogSplitter.LogSplitResult result = miningParameters.getLogSplitter().split(log, logInfo, cut, minerState);
